...
def load_agregated_recommendations(data_dir: str, fold: int, group: str, group_size: int) -> List[AlgRecommendations]:
    whitelist = ['GFAR_1.0', 'pref_AVG', 'pref_FuzzyDHondtDirectOptimize_1.0', 'pref_FuzzyDHondt_1.0']
...
def compute_metrics(fold, test_data: np.ndarray, groups: List[Group], group_weights: List[GroupWeights], alg_data: AlgRecommendations) -> List[Result]:
    ...
    i = 0    
    for group in groups:
        single_group_weights =  group_weights[i][1]
        ...
        if len(rec_for_group) >0:
          ...
          sum_ratings = np.sum(group_users_mean_ratings)
          sum_ndcg = np.sum(group_users_ndcg_ratings)
          
          proportional_user_mean_ratings = [i/sum_ratings for i in group_users_mean_ratings]
          proportional_user_ndcg = [i/sum_ndcg for i in group_users_ndcg_ratings]
          
          for k in range(len(proportional_user_mean_ratings)):
              results.append(Result(alg_data.alg_name, group_id, group_user_id, "AR", group_users_mean_ratings[k], single_group_weights[k], proportional_user_mean_ratings[k])   )
              results.append(Result(alg_data.alg_name, group_id, group_user_id, "nDCG", group_users_ndcg_ratings[k], single_group_weights[k], proportional_user_ndcg[k])   )
...
def process_fold(groups: List[Group], group_weights: List[GroupWeights], data_dir: str, fold: int, group: str, group_size: int) -> List[Result]:
    ...
    for alg_data in algs_data:
        results.extend(compute_metrics(fold, test_data, groups, group_weights, alg_data))
...
def main(data_folder, group_type, group_size):
    ...
    results = []
    for fold, _ in folds:
        results.extend(process_fold(groups, group_weights, data_folder, fold, group_type, int(group_size)))

        
    algs = set(map(lambda x:x.alg, results))
    metrics = set(map(lambda x:x.metric, results))
    res = "alg,group_id,user_id,metric,result,expected_share,result_share\n"
    for result in results:
