...
def load_agregated_recommendations(data_dir: str, fold: int, group: str, group_size: int) -> List[AlgRecommendations]:
    whitelist = ['GFAR_1.0', 'AVG-F', 'FuzzyDHondtDirectOptimize-F', 'FuzzyDHondtDirectOptimize_1.0',  'FuzzyDHondt-F']
    blacklist = "rec_rel"
...
def compute_metrics(test_data: np.ndarray, groups: List[Group],  alg_data: AlgRecommendations) -> List[Result]:
    ...
    for group in groups:
        ...
        for k in range(len(group_users_mean_ratings)):
            results.append(Result(alg_data.alg_name, group_id, group.members[k], "AR", group_users_mean_ratings[k])   )
            results.append(Result(alg_data.alg_name, group_id, group.members[k], "nDCG", group_users_ndcg_ratings[k])   )
